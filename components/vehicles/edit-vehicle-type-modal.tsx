...
import { updateVehicleType, type VehicleType } from "@/lib/vehicle-types"
...
export default function EditVehicleTypeModal({ isOpen, onClose, vehicleType, onSuccess }: EditVehicleTypeModalProps) {
  const [name, setName] = useState(vehicleType.name)
  const [description, setDescription] = useState(vehicleType.description)
  const [capacity, setCapacity] = useState(vehicleType.capacity.toString())
  const [seatingArrangement, setSeatingArrangement] = useState(vehicleType.seatingArrangement)
  const [isActive, setIsActive] = useState(vehicleType.isActive)
  const [status, setStatus] = useState<"active" | "maintenance" | "inactive">(vehicleType.status)
  const [errors, setErrors] = useState<Record<string, string>>({})

  // Update form when vehicleType changes
  useEffect(() => {
    setName(vehicleType.name)
    setDescription(vehicleType.description)
    setCapacity(vehicleType.capacity.toString())
    setSeatingArrangement(vehicleType.seatingArrangement)
    setIsActive(vehicleType.isActive)
  ...
  const handleSubmit = () => {
    ...
    // Update vehicle type
    updateVehicleType(vehicleType.id, {
      name,
      description,
      ...
      <DialogContent className="sm:max-w-[600px] p-0 overflow-auto max-h-[90vh]">
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold">Edit Vehicle Type</h2>
              <p className="text-muted-foreground">Modify an existing vehicle type configuration</p>
            </div>
